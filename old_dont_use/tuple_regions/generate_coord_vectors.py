...
# Hurricanes with fewer than 3 trajectories are not included in the list, but are recorded as pruned
import sys
...
def main():
  if len(sys.argv) != 4:
    print "Usage: {0} <StormsCsvInput> <VectorsOutput> <PrunedStormsOutput>".format(sys.argv[0])
  else:
    parseHurricanes()
